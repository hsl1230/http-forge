...
- **Enum-driven select dropdowns**: Path parameters, query parameters, and headers with `enum` arrays in `PathParamEntry`/`KeyValueEntry` metadata render as `<select>` dropdowns instead of text inputs. The `format` field provides validation patterns. For path params, metadata takes priority over URL-constraint-derived values. `path-variables-manager.js` merges metadata from `updateFromPath()` params argument; `query-params-manager.js` and `main.js` (`setupHeaders`) pass `enum`/`format` to `addParamRow`/`addHeaderRow`. Editing enum/format in the metadata panel immediately updates the value control (input↔select).
